...
Citizen.CreateThread(function()
    ...
    Koci.registerCallback("0r-towtruck:server:HasTaskBeenTaken", function(source, cb, taskId)
        local Player = Koci.getPlayer(source)
        if not Player then
            takenTasks[taskId] = nil
            return cb(false)
        end
        cb(takenTasks[taskId])
    end)
end)
...
RegisterServerEvent("0r-towtruck:server:OnStartJob", function(taskId)
    takenTasks[taskId] = true
end)
...
RegisterServerEvent("0r-towtruck:server:SaveLevelData", function()
    SaveLevelData()
end)
