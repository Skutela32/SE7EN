...
local function AddRadialOption()
    ...
    if ClosestGarage then
        ...
        garageJobName = garage.job or garage.gang
    end

    if garage then
        if not inVehicle and (garage.available or IsGarageOwner or IsKeyHolder) then
            if garageJobName and not CheckJob(garageJobName, garage.grade) then goto continue end
            ...
            if garage.type == 'plane' or (Config.EnablePublicInteriors and garage.type ~= 'boat' and not garage.isImpound and not inVehicle) then
                table.insert(menuItems, {
                    id = 'radialEnterShell',
                    title = 'Enter Garage',
                    icon = 'warehouse',
    ...
    for k, garage in pairs(Config.JobGarages) do
        ...
        local access = CheckJob(_job, garage.grade)
        if access then
